...
class LoginVC: UIViewController {
    ...
    struct defaultsKeys {
        static let keyOne = "tvlogin"
        static let keyTwo = "tvpassword"
        static let sip_log = "sip_log"
        static let sip_pass = "sip_pass"
        static let token = "token"
        static let dev_id = "dev_id"
    }
    ...
    override func viewDidLoad() {
        ...
        if (defaults.string(forKey: defaultsKeys.keyOne) != nil) {
            self.tvLogin.text = defaults.string(forKey: defaultsKeys.keyOne)
        }
        if (defaults.string(forKey: defaultsKeys.keyTwo) != nil) {
            self.tvPassword.text = defaults.string(forKey: defaultsKeys.keyTwo)
        }
                                ...
                                DispatchQueue.main.async {
                                    ...
                                    Alamofire.request("https://telefon.ufanet.ru/api/Users/Get?username="+self.tvLogin.text!, method: .get, headers: ["Authorization": self.token1]).responseJSON {
                                        ...
                                        if let value = response.result.value {
                                            ...
                                            self.defaults.set(self.tvLogin.text, forKey: defaultsKeys.keyOne)
                                            self.defaults.set(self.tvPassword.text, forKey: defaultsKeys.keyTwo)
                                            self.defaults.set(self.sip_login, forKey: defaultsKeys.sip_log)
                                            self.defaults.set(self.sip_pass, forKey: defaultsKeys.sip_pass)
                                            self.defaults.set(self.token, forKey: defaultsKeys.token)
                                            ...
                                            //(self.tabbar?.viewControllers?[2] as? SoftPhoneVC)?.token = self.token
